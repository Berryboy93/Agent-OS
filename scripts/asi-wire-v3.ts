...
const ROOT =
  process.env.ROOT_DIR || path.resolve(process.env.HOME || "", "Agent-OS")

type Finding = {
  file: string
  category: "STATE" | "EVENT" | "LEGACY" | "ARCHITECTURE"
  subtype: string
  line: number
  confidence: number
}
...
function validateFindings(list: Finding[]) {
...
fs.writeFileSync(
  path.join(ROOT, "audit/asi-wire-v3.json"),
  JSON.stringify(
    {
      root: ROOT,
