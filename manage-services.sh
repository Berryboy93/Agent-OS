...
readonly DASHBOARD_PORT=5173
readonly DASHBOARD_DIR="${PROJECT_DIR}/apps/dashboard"
readonly DASHBOARD_PID_FILE="${PROJECT_DIR}/.runtime/dashboard.pid"
readonly DASHBOARD_LOG="${PROJECT_DIR}/.logs/dashboard.log"

# Feature flags
readonly ENABLE_HEALTH_CHECKS=true
readonly HEALTH_CHECK_TIMEOUT=5
readonly HEALTH_CHECK_RETRIES=3
readonly LOG_ROTATION_SIZE=$((10 * 1024 * 1024))
...
stop_dashboard() {
  ...
  rm -f "$DASHBOARD_PID_FILE"
  print_status "Dashboard stopped (was PID: $pid)"
}

start_dashboard() {
  print_header "Starting Dashboard (port ${DASHBOARD_PORT})"
  ...
  print_status "Dashboard started (PID: $pid)"
