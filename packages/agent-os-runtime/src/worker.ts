...
async function deployAgent(agentId: string): Promise<void> {
  ...
  try {
    ...
    await db.update(agents)
      .set({ status: 'idle', completedAt: new Date() })      .where(eq(agents.id, agentId));
    ...
    if (attemptIncrement >= agent.maxAttempts) {
      ...
      await db.update(agents)
        .set({ status: 'error', errorLog: errorMsg, attemptCount: attemptIncrement })        .where(eq(agents.id, agentId));
      ...
      await db.update(agents)
        .set({ status: 'registered', errorLog: errorMsg, attemptCount: attemptIncrement })        .where(eq(agents.id, agentId));
