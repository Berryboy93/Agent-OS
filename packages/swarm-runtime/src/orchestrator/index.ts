...
import { DAGScheduler, ScheduleResult, NodeAssignment } from '../scheduler/index.js';
...
export class SwarmOrchestrator {
  ...
  async init(): Promise<void> {
    ...
    for (let i = 0; i < this.config.max_concurrent_agents; i++) {
      ...
      this.capabilityManager.registerProfile({
        ...
        name: `Swarm Worker ${i}`,
        capabilities: [
          { id: 'compute', resource: 'compute', action: 'execute', scope: 'sandbox', constraints: {} },
          { id: 'memory_read', resource: 'memory', action: 'read', scope: 'isolated', constraints: {} },
          { id: 'memory_write', resource: 'memory', action: 'write', scope: 'isolated', constraints: {} }
        ],
  ...
  async submitDAG(dag: DAG): Promise<string> {
    ...
    await this.eventStore.append('dag_submitted', {
      dag_id: dag.id,
      plan,
      schedule,
  ...
  private async executeJob(data: any): Promise<any> {
    ...
    try {
      for (const stage of schedule.partitions) {
        ...
        const stageResults = await Promise.allSettled(
          stage.map(async (nodeId: string) => {
            const assignment = schedule.assignments.find((a: NodeAssignment) => a.node_id === nodeId);
      ...
      // Success
      await this.eventStore.append('dag_completed', {
        dag_id,
        session_id: sessionId,
      ...
      await this.eventStore.append('dag_failed', {
        dag_id,
        session_id: sessionId,
