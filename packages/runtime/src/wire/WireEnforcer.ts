import { Mutation, WireCheckResult } from "./WireTypes.js"
...
export class WireEnforcer {
  private policy: WirePolicy
  private skewToleranceMsec: number
  private maxAgeMsec: number
  private maxPayloadBytes: number
  private maxTargetLength: number
  ...
  async validate(mutation: Mutation): Promise<EnforcerCheckResult> {
    ...
    if (
      ...
      return {
        ok: false,
        reason:
          "Invalid mutation: timestamp must be a positive finite number (milliseconds since epoch)",
      }
    }
    ...
    if (mutation.target !== undefined && mutation.target !== null) {
      if (typeof mutation.target !== "string") {
        return {
          ok: false,
          reason:
            "Invalid mutation: target must be a string (if provided)",
        }
      }
    ...
    try {
      const readBeforeWriteValid =
        await this.policy.validateReadBeforeWrite(mutation)
      if (!readBeforeWriteValid) {
        return {
          ok: false,
          reason:
            "Read-before-write constraint violated: state must be read before mutation",
        }
      }
    ...
    try {
      const sideEffectsValid = await this.policy.validateSideEffects(
        mutation
      )
      if (!sideEffectsValid) {
        return {
          ok: false,
          reason:
            "Side-effects constraint violated: unsafe cascading writes detected",
        }
      }
