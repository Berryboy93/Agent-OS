...
function rawSql(db: AgentOSDb): SqlClient {
...
export class AgentRunner {
  private readonly scheduler: InMemoryScheduler;
  ...
  async run(input: RunInput): Promise<AgentResult> {
    ...
    return new Promise<AgentResult>((resolve, reject) => {
      this.pending.set(runId,
        { resolve, reject, startedAt: now },  );
      this.scheduler.add("run", jobData, { priority: 0 });
    });
  }

  private async runWorker(jobData: WorkerJobData): Promise<void> {
    this.activeRuns++;
    const sql = rawSql(this.db);
    ...
    const worker = new Worker(WORKER_ENTRY, {
      env: {
        ...process.env,
        NODE_PATH:
          "/home/r3v/Agent-OS/node_modules:/home/r3v/Agent-OS/packages/core/node_modules:/home/r3v/Agent-OS/packages/adapters/node_modules",
      },
      execArgv: ["--import", "tsx"],
      workerData: jobData,
      resourceLimits: {
      ...
      (msg: {
        ...
        if (msg.type === "event" && msg.event) {
          ...
          if (this.options.onEvent)
            this.options.onEvent(msg.event as AgentEvent);
        }
        ...
        `,
            ...
            .run(
              ...
              run.error ?? null,
              run.tokenUsage?.inputTokens ?? 0,
              run.tokenUsage?.outputTokens ?? 0,
    ...
    await new Promise<void>((resolve, reject) => {
      worker.on("exit", (code) => {
        ...
        if (p && finalRun) {
          ...
          this.pending.delete(jobData.runId);
          const err = new Error(
            `Worker exited with code ${code} without sending done`,
          );
          p.reject(err);
          reject(err);
        } else {
  ...
  async recoverCrashedRuns(): Promise<number> {
