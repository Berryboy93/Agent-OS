...
export class EndToEndPipeline {
  ...
  constructor(config: PipelineConfig) {
    ...
    // Initialize swarm
    // Note: In production, inject proper dependencies
    // this.swarm = new SwarmOrchestrator(config.swarm_config, ...);
  ...
  async process(dag: DAG): Promise<PipelineResult> {
    ...
    try {
      ...
      if (this.simulator) {
        ...
        if (!simulationPassed) {
          await this.eventStore.append({
            type: 'dag_rejected_simulation',
            payload: { dag_id: dagId, confidence: simResult.confidence },
            dag_id: dagId
          });
          return {
            dag_id: dagId,
      ...
      if (!mythosDecision.allowed) {
        await this.eventStore.append({
          type: 'dag_rejected_mythos',
          payload: {
            dag_id: dagId,
            rule: mythosDecision.rule,
            reason: mythosDecision.reason
          },
          dag_id: dagId
        });
        return {
